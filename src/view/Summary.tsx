...
const formatDate = (date: Date) => {
  return (
    Util.addLeadingZero(date.getDate()) +
    '/' +
...
export default class Summary extends Component<IProps, IState> {
  ...
  renderRow(occurrence: ISummaryOccurrence, index: number) {
    ...
    return (
      <tr
        id={occurrence.dark ? 'dark' : 'bright'}
        key={index}
        onClick={() => this.setState({ selected: occurrence })}
      >
      ...
      <Fragment>
        <table className='Summary'>
          <tbody>
            ...
            {alternateDays(this.props.occurrences)
        ...
        {this.fullyLoaded() || (
          <Icon
            ...
            onClick={() =>
              this.setState({ max: this.state.max + ROW_INCREMENT })
            }
          />
        )}
        {this.state.selected && (
