...
const zoom = (
  timeDomain: d3.ScaleTime<number, number>,
  ...
  bars
    ...
    .attr('x', o => {
      ...
      return (
        timeDomain(o.millis) +
        (o.skull.index *
          (timeDomain(dayEnd.getTime()) - timeDomain(o.millis))) /
          skulls.length
      );
    })
    .attr('width', o => {
      ...
      dayEnd.setDate(dayEnd.getDate() + 1);
      return (
        (timeDomain(dayEnd.getTime()) - timeDomain(o.millis)) / skulls.length
      );
    });
};
...
class DateFormatter {
  ...
  static format = (
    value: number | Date | { valueOf(): number },
    index: number
  ) => {
    const date = new Date(value.valueOf());

    return (
      d3.timeDay(date) < date && index > 0
        ? DateFormatter.formatEmpty
        : d3.timeWeek(date) < date
        ? DateFormatter.formatDay
        : d3.timeMonth(date) < date
        ? DateFormatter.formatWeek
...
export default class Chart extends PureComponent<IProps> {
  ...
  getSizesAndOrientation(margin: number) {
    ...
    const width =
      orientation === Orientation.HORIZONTAL ? screenWidth : screenHeight;
    const height =
      orientation === Orientation.HORIZONTAL ? screenHeight : screenWidth;
  ...
  componentDidMount() {
    ...
    // Sizes
    const { width, height, plotWidth, plotHeight, orientation } =
      this.getSizesAndOrientation(margin);
    ...
    const plot =
      orientation === Orientation.HORIZONTAL
        ...
        : chart.attr(
            'transform',
            `rotate(90, 0, ${height}) translate(-${height}, 0)`
          );
    ...
    const dayWidth =
      timeDomain(new Date(0, 0, 1).getTime()) -
      timeDomain(new Date(0, 0, 0).getTime());
    ...
    const brush = d3
      ...
      .on('end', event => {
        ...
        zoom(
          ...
          bars,
          this.props.skulls,
          timeDomain.invert(extent[0]),
          timeDomain.invert(extent[1]),
          false
        );
      });
